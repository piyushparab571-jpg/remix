...
The setNumber() function allows any external caller to modify the
storedNumber state variable because no access control mechanism is 
implemented.
...
since the function is declared external without authorixation checks, any user 
can arbitrarily update protocol state.

Impact
An attacker can overwite the stored value with arbitrary data,
if this variable controlled critical protocol logic such as:
1. pricing configuration
2. treasury parameters
3. protocol state variables
4. governance-controlled values
5. system execution thresholds
then unauthorized users could manipulateoverall protocol behavoir.

Proof of concept
Step 1:-Deploy contract
Deploy the vulnerable DataStorage contract.

Step 2:- Legitimate User Update State
User A calls:
setNumber(100)
State becomes:
storedNumber = 100

Step 3:- Attacker Overwrites State
Attacker calls:
setNumber(999999)
State changes successfully:
storedNumber = 999999
No restriction prevents unauthorized modification.

Root Cause
The function lacks authorization validation
Specifically:
no ownership mechanism exists
no role-based access control is implemented
no require() statement validates caller permissions
the function is publicly accessible to all external users

Recommendation
Restric access to authorized users only.

implement an owner variable and validate the caller before allowing state
modification.
Example:
