...
/*
Audit Report
Title: Unsafe Low-Level External Calls Without Proper Failure
Handling

Severity: High

Reason:
The contract performs low-level external calls using .call() and relies on manual
success validation.

Unchecked or improperly handled external calls can:
1. silently fail
2. corrupt protocol state
3. cause inconsistent accounting
4. hide revert reasons
5. break execution assumptions
...
(bool success, bytes momory data)
if:
1. success is ignored
2. revert data is not decoded
3. failures are not emitted/logged
then debugging and seurity monitoring become difficult.
Additionally, low-level calls do not automatically bubble revert reasons.

Impact:
Without proper handling:
1. protocol may continue after failed execution
2. revert causes become hidden
3. ETH transfer may silently fail
4. token transfers may fail undetected
5. monitoring systems lose visibility

Root Cause:
The contract uses raw .call() patterns without:
1. revert decodein
2. event emission
3. structured try/catch logic
4. comparision between safe and unsafe flows

Recommendation:
Implement:
1. try/catch wrappers
2. revert decoding helpers
3. failure event logging
4. checked external calls
5. ERC20 safe transfer wrappers
...
contract SafeCallHandler {
    ...
    event UncheckedCallFailed(bytes data);
    ...
    function checkedCall(address target) external {
        try CallTarget(target).successFunction() {
            lastSuccess = true;
        }
    ...
    function lowLevelCall(address target) external {
        (bool success, bytes memory data) =
            target.call(
                abi.encodeWithSignature(
                    "failFunction()"
    ...
    function _getRevertMsg(bytes memory data)
        internal
        pure
    ...
    /*//////////////////////////////////////////////////////////////
                    CHECKED vs UNCHECKED CALLS
    //////////////////////////////////////////////////////////////*/

    function checkedETHTransfer(address payable to)
        external
        payable
    {
    ...
    // dangerous: ignores failure
    function uncheckedETHTransfer(address payable to)
        external
        payable
    {
