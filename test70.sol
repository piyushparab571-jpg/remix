...
/*
Audit Report
Title; Unbounded Calldata Processing Can Lead to Excessive Gas Consumption

Severity: Medium

Reason
The contract processes an arbitrary-length calldata array and performs a storage write
during every iteration. Large arrays can significantly gas consumption and
may cause transactions to run out of gas.
...
Problems:
- No maximum array size limit.
- Storage write on every iteration.
- Gas usage scales linearly with input size
Suppling very large arrays can make execution expensive.
...
Root Cause
Repeated storage updates inside the loop:
totalElements++;
combined with unbounded calldata input.

Recommendation
- Limit calldata array size.
- Remove storage writes from inside loops.
- Batch process large arrays.
- Use a singlestorage update after loop completion.
...
contract CalldataGas {
    uint256 public totalSum;
    uint256 public totalElements;
    uint256 public gasUsed500;
    uint256 public gasUsed1000;
    /*
    ========================
    PROCESS MAX 500 ELEMENTS
    ========================
    */
    function processCalldataArray(
        uint256[] calldata data
    ...
    /*
    =====================================
    COMPARE 1000 ELEMENT GAS
    =====================================
    */
    function compare1000Gas(
        uint256[] calldata data
    )
    external
    {
